...
- `보조 테이블` 은 ngram과 같은 특별한 parser를 사용할 때 성능 향상을 기대할 수 있으며 사용하려면 직접 지정해줘야 한다. (필수아니라는뜻)
    - 방법
        
        ```sql
        SET GLOBAL innodb_ft_aux_table = 'mydb.table_name'
        ```
        
- MySQL은 실제 데이터 테이블에 대한 텍스트 데이터를 인덱싱하는데, 이 때 생성된 인덱스 정보를 보조 테이블에 저장한다.
- MySQL에서 `FULLTEXT` 인덱스를 사용할 때 해당 인덱스의 정보를 `보조 테이블` 에 저장한다. 이 테이블은 인덱스의 단어, 해당 단어가 등장하는 문서 위치, 문서 내 단어의 위치 등의 정보를 저장한다. 이 정보를 통해 full-text 검색이 더 빠르고 효율적으로 작동하게 된다.
- `보조 테이블` 은 MySQL의 `인덱스 캐시` 역할을 하며 실제 데이터 테이블에서 직접 검색하는 것보다 `보조 테이블` 에서 인덱스를 조회하여 더 빠르게 결과를 반환한다.
